...
function OrderConfirmation() {
  ...
  const getNextSteps = (paymentType) => {
    if (paymentType === 'escrow') {
      return [
        "Meet with seller at the agreed location",
        "Inspect the item carefully",
        "Confirm delivery in your Orders page to release payment"
      ];
    }
    return [
      "Meet with seller at the agreed location",
      "Payment has been sent to the seller",
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Order #{orderId} • {mockOrder.createdAt}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="space-y-4 border-t border-gray-200 pt-4">
            ...
            <div className="flex items-center gap-2 text-gray-600">
              ...
              <span>Payment via: {mockOrder.paymentType === 'escrow' ? 'Escrow (Payment held until delivery confirmed)' : 'Direct Payment'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <ol className="space-y-4">
            {getNextSteps(mockOrder.paymentType).map((step, index) => (
              <li key={index} className="flex items-start gap-3">
                <span className="flex-shrink-0 w-6 h-6 bg-[#FFB800] text-black rounded-full flex items-center justify-center text-sm">
                  {index + 1}
                </span>
